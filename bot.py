...
# تخطي حماية Cloudflare لموقع مستقل ونفذلي
...
def fetch_mostaql():
    projects = []
    try:
        # استخدام scraper للتخفي وتخطي الحظر
        res = scraper.get("https://mostaql.com/projects", timeout=15)
        soup = BeautifulSoup(res.text, 'html.parser')
        for link in soup.find_all('a', href=True):
            href = link['href']
            # استبعاد روابط إنشاء مشروع مشابه لضمان الدقة
            if 'mostaql.com/project/' in href and not href.endswith('/bids') and '/create?' not in href:
                title = link.text.strip()
                if title and len(title) > 5:
                    p_id = href.split('/')[-1].split('-')[0]
                    projects.append({'id': f"mostaql_{p_id}", 'title': title, 'url': href, 'platform': 'مستقل'})
    except Exception as e:
        print("Mostaql Error:", e)
    return projects

def fetch_nafezly():
    projects = []
    try:
        res = scraper.get("https://nafezly.com/projects", timeout=15)
        soup = BeautifulSoup(res.text, 'html.parser')
        for link in soup.find_all('a', href=True):
            href = link['href']
